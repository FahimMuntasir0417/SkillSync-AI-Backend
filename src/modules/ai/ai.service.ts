...
const logAiRequest = (payload: LogAiRequestInput) =>
  prisma.aiRequestLog.create({
    data: {
      ...
      response: payload.response === undefined ? undefined : (payload.response as Prisma.InputJsonValue),
...
const assertAiProviderConfigured = () => {
  if (!env.OPENAI_API_KEY && !env.GEMINI_API_KEY) {
    throw new AppError(httpStatus.SERVICE_UNAVAILABLE, "AI provider is not configured");
  }
};
...
const runAiTask = async <T>(userId: string, feature: AiFeatureType, prompt: string, shape: string) => {
  try {
...
const generateCourseSummary = async (payload: CourseSummaryInput, userId: string) => {
  const course = await prisma.course.findUnique({
    ...
    select: {
      ...
      description: true,
      level: true,
      modules: { select: { title: true, lessons: { select: { title: true, content: true } } }, orderBy: { order: "asc" } },
    },
  });
  if (!course) throw new AppError(httpStatus.NOT_FOUND, "Course not found");
  return runAiTask(userId, AiFeatureType.COURSE_SUMMARY, `Summarize this course context: ${JSON.stringify(course)}`, "{ summary: string; difficulty: string; outcomes: string[]; recommendedFor: string[] }");
};

const chat = async (payload: ChatInput, userId: string) => {
  const [course, lesson] = await Promise.all([
    payload.courseId ? prisma.course.findUnique({ where: { id: payload.courseId }, select: { title: true, description: true } }) : null,
    payload.lessonId ? prisma.lesson.findUnique({ where: { id: payload.lessonId }, select: { title: true, content: true } }) : null,
  ]);
  return runAiTask(userId, AiFeatureType.STUDY_ASSISTANT, `Question: ${payload.message}\nCourse context: ${JSON.stringify(course)}\nLesson context: ${JSON.stringify(lesson)}`, "{ answer: string; suggestedQuestions: string[] }");
};

const recommendations = async (payload: RecommendationInput, userId: string) => {
  const [enrollments, completedLessons, courses] = await Promise.all([
    prisma.enrollment.findMany({ where: { userId }, select: { course: { select: { title: true, category: { select: { name: true } } } } } }),
    prisma.lessonProgress.count({ where: { userId, completed: true } }),
    prisma.course.findMany({ where: { status: "PUBLISHED" }, select: { id: true, title: true, shortDescription: true, category: { select: { name: true } } }, take: 20 }),
  ]);
  return runAiTask(userId, AiFeatureType.RECOMMENDATION, `Interest: ${payload.interest ?? ""}\nEnrollments: ${JSON.stringify(enrollments)}\nCompleted lessons: ${completedLessons}\nAvailable courses: ${JSON.stringify(courses)}`, "{ recommendations: { courseId: string; title: string; reason: string }[] }");
};

const assignmentFeedback = async (payload: AssignmentFeedbackInput, userId: string) => {
  const submission = payload.submissionId
    ? await prisma.submission.findUnique({ where: { id: payload.submissionId }, select: { githubUrl: true, liveUrl: true, notes: true, assignment: { select: { title: true, description: true } } } })
    : null;
  if (payload.submissionId && !submission) throw new AppError(httpStatus.NOT_FOUND, "Submission not found");
  return runAiTask(userId, AiFeatureType.ASSIGNMENT_FEEDBACK, `Review assignment submission: ${JSON.stringify(submission ?? payload)}`, "{ scoreEstimate: number; strengths: string[]; improvements: string[]; finalComment: string }");
};
...
const getAiLogs = async (query: AiLogQuery) => {
  ...
  const safeSortBy = aiLogSortableFields.includes(sortBy as (typeof aiLogSortableFields)[number]) ? sortBy : "createdAt";
  const where: Prisma.AiRequestLogWhereInput = {
    ...(typeof query.feature === "string" ? { feature: query.feature as AiFeatureType } : {}),
    ...(typeof query.status === "string" ? { status: query.status as AiRequestStatus } : {}),
    ...(typeof query.userId === "string" ? { userId: query.userId } : {}),
  };
  const [total, logs] = await Promise.all([
    prisma.aiRequestLog.count({ where }),
    prisma.aiRequestLog.findMany({ where, select: aiLogSelect, skip, take: limit, orderBy: { [safeSortBy]: sortOrder } }),
  ]);
  return { meta: { page, limit, total, totalPage: Math.ceil(total / limit) }, data: logs };
};
